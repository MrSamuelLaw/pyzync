...
# define the adapter config, this will be imported into main.py CLI app.
GB = 2**30  # 1 GB
ADAPTERS = [
    LocalFileStorageAdapter(name="USB", directory=environ['FILE_BACKUP_DIR']),
    DropboxStorageAdapter(name="DBX",
                          directory=environ['DROPBOX_DIR'],
                          access_token=environ['DROPBOX_TOKEN'])
]

backup_config = {
    'tank1/foo': BackupJob(retention_policy=RollingNSnapshotsPolicy(n_snapshots=5), adapters=ADAPTERS)
}
...
async def main():
    ...
    for dataset_id, job in backup_config.items():
        ...
        headers = [
            'dataset', 'timestamp', 'producer sts', 'producer dur', 'consumer', 'consumers sts',
            'consumers dur'
        ]
        for sm in stream_managers:
            rows.append([
                ...
                humanize.naturaldelta(sm.producer.total_seconds), '\n'.join(
                    (c.generator.gi_code.co_qualname.split(".")[0] for c in sm.consumers)), '\n'.join(
                        (c.exit_status for c in sm.consumers)), '\n'.join(
                            (humanize.naturaldelta(c.total_seconds) for c in sm.consumers))
            ])
