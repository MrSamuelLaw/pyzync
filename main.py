#!./.venv/bin/python
"""Interactive CLI for PyZync with home menu for backup/restore operations."""
...
def select_adapter() -> tuple[str, RemoteSnapshotManager]:
    ...
    """
    adapters = backup.ADAPTERS
    console.print("\n[bold]Available Backup Adapters:[/bold]")
    for idx, adapter in enumerate(adapters, 1):
        console.print(f"  {idx}. {adapter.name}")

    choice = Prompt.ask("Select adapter", choices=[str(i) for i in range(1, len(adapters) + 1)])
    selected_adapter = adapters[int(choice) - 1]
    manager = RemoteSnapshotManager(adapter=selected_adapter)

    console.print(f"[green]Selected: {selected_adapter.name}[/green]")
    return selected_adapter.name, manager
...
def restore_main() -> None:
    ...
    """
    console.print("[bold blue]PyZync Restore[/bold blue]")
...
def backup_main() -> None:
    """Main entry point for the backup workflow.

    Runs the backup job defined in backup.py.
    """

    console.print("[bold blue]PyZync Backup[/bold blue]")
    console.print("\n[cyan]This will run all configured backup jobs.[/cyan]")
...
def home_menu() -> None:
    """Home menu: display options and dispatch to selected workflow."""
    console.print("\n[bold blue]PyZync[/bold blue]")
    console.print("[cyan]Choose an operation:[/cyan]")
    console.print("  1. Restore from Backup")
    console.print("  2. Run Backup Job")

    choice = Prompt.ask("Select operation", choices=["1", "2"])
